...
setup:
	...
	pip install git+https://github.com/relations-dil/python-relations.git@3f8ce8b#egg=python-relations && \
	pip install git+https://github.com/relations-dil/python-relations-sql.git@15e3ad4#egg=python-relations-sql && \
	python setup.py install && \
	python -m relations_sqlite.sql && \
	python -m relations_sqlite.expression && \
	python -m relations_sqlite.criterion && \
	python -m relations_sqlite.criteria && \
	python -m relations_sqlite.clause && \
	python -m relations_sqlite.query && \
	python -m relations_sqlite.ddl && \
	python -m relations_sqlite.column && \
	python -m relations_sqlite.index && \
	python -m relations_sqlite.table"
